...
/*Function Pointers to Functions in libpowerusb.dylib starts here*/

 int (*InitPowerUSB_Address)(int *, char []);
 int (*CheckStatusPowerUSB_Address)(void);
 int (*SetCurrentPowerUSB_Address)(int);
 int (*StopWatchdogTimerPowerUSB_Address)(void);
 int (*StartWatchdogTimerPowerUSB_Address)(int , int , int );
 int (*GetInputStatePowerUSB_Address)(int input[]);
 int (*SetOutputStatePowerUSB_Address)(int output[]);
 int (*ClearPLCPowerUSB_Address)(void);
 int (*SetPLCPowerUSB_Address)(int);
 int (*SetInputTriggerPowerUSB_Address)(int, int, int, int, int, char, char);
 int (*ReadPortStatePowerUSB_Address)(int *, int *, int *);
 int (*SetPortPowerUSB_Address)(int, int, int);
 int (*ReadCurrentPowerUSB_Address)(int *);
 int (*ReadCurrentCumPowerUSB_Address)(int *);
 int (*SendHeartBeatPowerUSB_Address)(void);
 
/*Function Pointers to Functions in libpowerusb.dylib ends here*/
...
int main(int argc, char* argv[])
{
	...
	if (!current && !watchdog && !digOut && !digIn && !trigAction && (!numOutlets || (numOutlets%3)))
	{
		printf("Usage: %s [0,1] [0,1] [0,1] p c (0 for off and 1 for on) \r\n", argv[0]);
    ...
    {
		(*SetCurrentPowerUSB_Address)(0);
        printf("PowerUSB Connected. Number of Devices Connected : %d . Model: %d. Version:%s\n", MaxUnits, model, firmware);
    }
	...
	if (watchdog)											// Watchdog operation. quit after watchdog function
	{
		...
		{
			(*StopWatchdogTimerPowerUSB_Address)();
        ...
        (*StartWatchdogTimerPowerUSB_Address)(watchOpt[0], watchOpt[1], watchOpt[2]);			// start watchdog with given parameters
	...
	{
		r = (*GetInputStatePowerUSB_Address)(ioStates);
	...
	if (digOut)													// Set digital outputs. Make sure 3 output parameters are given
	{
		...
		r = (*SetOutputStatePowerUSB_Address)(ioStates);
	...
	if (trigAction)												// Set a trigger action for an input change
	{
		...
		for (i = 2; i < 8; i++)
		{
			...
			if (trigParam[i] != -3)							// set actions only for parameters that are not -3(-3=no action)
			{
				...
				(*SetInputTriggerPowerUSB_Address)(input, loToHi, pwrOutlet, output, trigParam[i], cond, mask);		// convert condition to 2 bytes understood by PLC
	...
	if (numOutlets > 0)
	{
		(*ReadPortStatePowerUSB_Address)(&in1, &in2, &in3);			// Read and display the state of outlets before change
		printf("Before Setting Port Status: %d %d %d\n", in1, in2, in3);
	}
	if (numOutlets >= 3)									// 1st PowerUSB (default)
	{
		(*SetPortPowerUSB_Address)(outlet[0], outlet[1], outlet[2]);	// set the oulets to on or off as per parameters
	...
	if (current)												// Read and display power consumption information
	{
		...
		r2 = (*ReadCurrentCumPowerUSB_Address)(&currentCum);				// Cumulative curretn consumption in amps/minute
...
int SendHeartbeat(int interval)
{
	...
	while (1)
	{
		(*SendHeartBeatPowerUSB_Address)();					// Send heartbeat 
	...
	(*StopWatchdogTimerPowerUSB_Address)();					// Stop the watchdog timer since we are stopping the heartbeat
	return 0;
}
...
void loadSharedLibrary()
{
	//sharedLibraryHandle = dlopen ("./libpowerusb.dylib", RTLD_LAZY);
    string dynLibPath = ofToDataPath("libpowerusb.dylib");
    cout<<dynLibPath<<endl;
    sharedLibraryHandle = dlopen(dynLibPath.c_str(), RTLD_LAZY);
    if (!sharedLibraryHandle) 
    {
        fputs (dlerror(), stderr);
        printf("\nMissing Required Library libpowerusb.dylib");
        sharedLibraryHandle = NULL;
        exit(1);
     }
    else
    {
        InitPowerUSB_Address = (int (*)(int *, char []))dlsym(sharedLibraryHandle, "InitPowerUSB");
        CheckStatusPowerUSB_Address = (int (*)(void))dlsym(sharedLibraryHandle, "CheckStatusPowerUSB");
        SetCurrentPowerUSB_Address = (int (*)(int))dlsym(sharedLibraryHandle, "SetCurrentPowerUSB");
        StopWatchdogTimerPowerUSB_Address = (int (*)(void))dlsym(sharedLibraryHandle, "StopWatchdogTimerPowerUSB");
        StartWatchdogTimerPowerUSB_Address = (int (*)(int, int, int))dlsym(sharedLibraryHandle, "StartWatchdogTimerPowerUSB");
        SendHeartBeatPowerUSB_Address = (int (*)(void))dlsym(sharedLibraryHandle, "SendHeartBeatPowerUSB");
        GetInputStatePowerUSB_Address = (int (*)(int []))dlsym(sharedLibraryHandle, "GetInputStatePowerUSB");
        SetOutputStatePowerUSB_Address = (int (*)(int []))dlsym(sharedLibraryHandle, "SetOutputStatePowerUSB");
        ClearPLCPowerUSB_Address = (int (*)(void))dlsym(sharedLibraryHandle, "ClearPLCPowerUSB");
        SetPLCPowerUSB_Address = (int (*)(int))dlsym(sharedLibraryHandle, "SetPLCPowerUSB");
        SetInputTriggerPowerUSB_Address = (int(*)(int, int, int, int, int, char, char))dlsym(sharedLibraryHandle, "SetInputTriggerPowerUSB");    
        ...
        ReadCurrentPowerUSB_Address = (int (*)(int *))dlsym(sharedLibraryHandle, "ReadCurrentPowerUSB");
        ReadCurrentCumPowerUSB_Address = (int (*)(int *))dlsym(sharedLibraryHandle, "ReadCurrentCumPowerUSB");
        SendHeartBeatPowerUSB_Address = (int (*)(void))dlsym(sharedLibraryHandle, "SendHeartBeatPowerUSB");
    }
}
